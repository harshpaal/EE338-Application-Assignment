...
%Normalize Data
if(unorm==1) %relative normalisation
    if (min(mag(:))<0)
        error('Some values are negative!!');
        return
    end
    nmin = 0.50*min(mag(:));
    mag = mag - nmin;
    mag = mag./(max(mag(:)));
    %mag= mag./(max(mag(:)));
elseif(unorm==2) %absolute value for percentage
    if(max(mag(:)>100))
        error('Some values are over 100%!!');
        return
    elseif(min(mag(:)<0))
        error('Some values are negative%!!');
        return
